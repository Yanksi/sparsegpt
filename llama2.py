...
def llama_sequential(model, dataloader, dev):
    ...
    model = model.to(dev)
    linear_layers = get_layers_with_type(layers, nn.Linear, prefix='model.layers.')
...
@torch.no_grad()
def llama_gmp(model, target_sparsity):
    print("Starting ...")
    layers = model.model.layers
    linear_layers = get_layers_with_type(layers, nn.Linear, prefix='model.layers.')
    masks = {}
...
def llama_eval_all(model, tag):
    ...
    for dataset in ['wikitext2', 'ptb', 'c4']:
        ...
        print(dataset)
        perplexity = llama_eval(model, testloader, DEV)
...
if __name__ == "__main__":
    ...
    if args.baseline:
        perplexities['dense'] = llama_eval_all(model, "dense")

    if (args.sparsity or args.prunen):
        model = get_llama(args.model)
        model.eval()
        # tick = time.time()
        masks = llama_sequential(model, dataloader, DEV)
        if args.save:
            model.save_pretrained(args.save)
            torch.save(masks, f'{args.save}/masks.pt')
        perplexities['SparseGPT'] = llama_eval_all(model, "SparseGPT")
